...
const googleApi = {
  ...
  loadGoogleAPI() {
    return this.get()
      .then(() => {
        if (
          store.getState().profile.googleAPIState ===
          GOOGLE_API_STATES.NEEDS_LOADING
        ) {
          return googleApi.initializeClient(
            '621897095595-k7tr68mgfrhm1935cqdq5l2vg8u7ltu8.apps.googleusercontent.com',
            true,
            true,
          );
        }
  ...
  isSignedIn() {
    return this.get().then(api =>
      Boolean(api?.auth2?.getAuthInstance()?.isSignedIn?.get()),
    );
  },
  ...
  async requestLiveStreamsForYouTubeBroadcast(boundStreamID) {
    return this.get().then(api =>
      api.client.request(`${API_URL_BROADCAST_STREAMS}${boundStreamID}`),
    );
  },
