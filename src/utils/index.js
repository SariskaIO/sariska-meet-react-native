...
export async function getToken(profile, name, avatarColor) {
  ...
  console.log('body', body, GENERATE_TOKEN_URL);
  try {
    const response = await fetch(GENERATE_TOKEN_URL, body);
    console.log('respon', response);
    if (response.ok) {
      const json = await response.json();
      AsyncStorage.setItem('SARISKA_TOKEN', json.token);
      console.log('json', json, json.token);
      return json.token;
    } else {
...
export function getPresignedUrl(params) {
  return new Promise((resolve, reject) => {
    const body = {
      ...
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${AsyncStorage.getItem('SARISKA_TOKEN')}`,
      },
      body: JSON.stringify({
