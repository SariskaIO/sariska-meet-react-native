...
export const microsoftCalendarApi = {
  ...
  signIn() {
    ...
    popupAuthWindow = window.open(
      authUrl,
      'Auth M$',
      `width=${w}, height=${h}, top=${window.screen.height / 2 - h / 2}, left=${
  ...
  _isSignedIn() {
    ...
    const msAuthState = state.msAuthState || {};
    const tokenExpires = parseInt(msAuthState.tokenExpires, 10);
...
function b64utoutf8(str) {
  ...
  s = bytes.reduce(
    (str_, byte) => str_ + byte.toString(16).padStart(2, '0'),
    '',
  );
